<?php
    ...
    if($method == "GET"){
        ...
        if($params_p["all"] == "false")
            echo json_encode(array("response" => $artistServ->getPreference($ID), "HttpCode" => 200, "datetime" => new datetime()));
    }else if ($method == "UPDATE"){
        if(explode("=", $params[2])[0] == "for" && explode("=", $params[2])[1] == "ADD_PREF"){
            // www.domain.com?controllers=artist&method=UPDATE&for=ADD_PREF&artist_id=:artist_id
            ...
            // www.domain.com?controllers=artist&method=UPDATE&for=REMOVE_PREF&artist_id=:artist_id
            $artist_id = $params_p["artist_id"];
            $user_id = $ID;
