...
if($method == "GET"){
    ...
    // www.domain.com?controllers=playlist&method=UPDATE&action=ADD&music_id=:music_id&playlist_id=:playlist_id
    if($params_p["action"] == "ADD")
        echo json_encode(array("response" => array("add" => $playlistServ->addSong($params_p["music_id"],$params_p["playlist_id"])), "HttpCode" => 200, "datetime" => new datetime()));
    // www.domain.com?controllers=playlist&method=UPDATE&action=REMOVE&music_id=:music_id&playlist_id=:playlist_id
    else if($params_p["action"] == "REMOVE")
        echo json_encode(array("response" => array("remove" => $playlistServ->removeSong($params_p["music_id"],$params_p["playlist_id"])), "HttpCode" => 200, "datetime" => new datetime()));
...
// www.domain.com?controllers=playlist&method=PUT&name=:name
    ...
    echo json_encode(array("response" => array("add" => $playlistServ->add($params_p["name"],$ID)), "HttpCode" => 200, "datetime" => new datetime()));
// www.domain.com?controllers=playlist&method=PUT&playlist_id=:playlist_id
