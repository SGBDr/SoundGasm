<?php
   ...
   class PlaylistRepo{
    ...
    public function findById(int $playlist_id): PlayList {
        ...
        $musics = array();
        $stmt = $this->con->prepare("SELECT m.music_id,m.name,m.rep_image,m.track,m.artist,m.style,m.country,m.release_date FROM m musics, mp music_playlist WHERE m.music_id = mp.music_id AND mp.playlist_id = :playlist_id");
    ...
    public function addSongIntoPlaylist(int $music_id, int $playlist){
    ...
    public function removeSongFromPlaylist(int $music_id, int $playlist){
